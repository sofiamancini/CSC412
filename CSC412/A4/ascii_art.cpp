...
void disableRawMode() {
    static bool rawModeDisabled = false;
    if (!rawModeDisabled) {
        termios term;
        tcgetattr(STDIN_FILENO, &term);
...
void keyListener() {
    ...
    // Disable raw mode when the thread exits
    disableRawMode();
}
...
void myKeyboard(unsigned char c) {
	...
	switch (c) {
        ...
        case '<':
            std::cout << "Slowing down the producers" << std::endl;
            slowdownProducers();
            break;
            
        // speed up the production of ink for the producer threads
        case '>':
            std::cout << "Speeding up the producers" << std::endl;
            speedupProducers();
            break;
            
		// test red ink up/down
		case 'r':
            std::cout << "Refilling red ink" << std::endl;
			ok = refillRedInk(MAX_ADD_INK);
			break;

        // make inklings move faster
        case '+':
            std::cout << "Speeding up the inklings" << std::endl;
        ...
        case '-':
            std::cout << "Slowing down the inklings" << std::endl;
		...
		case 'g':
            std::cout << "Refilling green ink" << std::endl;
			ok = refillGreenInk(MAX_ADD_INK);
			break;

		// test blue ink up/down
		case 'b':
            std::cout << "Refilling blue ink" << std::endl;
	...
	if (!ok) {
        ...
        std::cerr << "ERROR :/ myKeyboard :: unable to refill ink" << std::endl;
	}
...
void myEventLoop(int val) {
    ...
    updateTerminal();
    customTimerFunc(500, myEventLoop, val);
...
void initializeFrontEnd(int argc, char** argv, void (*gridDisplayCB)(void), void (*stateDisplayCB)(void)) {
    ...
    // Register disableRawMode to be called on program exit
    std::atexit(disableRawMode);

    // Initialize listenerThread only if it is not already running
