...
void assert_string_equal(char* expected, char* actual, char* message) {
    ...
    if (!strcmp(expected, actual)) {
        ...
        tests_incorrect++;
        printf("\033[0;31m"); // set color to red
        printf("[FAILED Test %d: %s]: Actual string: '%s', Expected string: '%s'\n",
...
// First check if the input has given a number of elements, run all tests if not
...
// If the input has given a number of elements, run the reverse function on the input

} else {
    int num_elements = atoi(argv[1]);

    // Check if the number of elements matches the number of elements in the array, if not, print an error message
